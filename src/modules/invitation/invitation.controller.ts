...
} from './dto/get-invitation-by-public-id.dto';
...
export class InvitationController {
  ...
  @Get(':id')
  async getByPublicId(
    @Param() { id: publicId }: GetInvitationByPublicIdParams,
    @Query() query: GetInvitationByPublicIdQuery,
  ) {
    return await this.invitationService.getByPublicId(publicId, query);
  }
  ...
  async acceptByPublicId(
    ...
    @Query() query: AcceptInvitationByPublicIdQuery,
  ) {
    return await this.invitationService.acceptByPublicId(publicId, data, query);
  }
}
